...
def get_data():
    ...
    df.columns = [c.strip() for c in df.columns] # কলামের নামের বাড়তি স্পেস মোছা
    return df
...
try:
    ...
    product_selected = st.sidebar.selectbox("Product Type", ["All", "Floorplan Queue", "Measurement Queue", "Autocad Queue", "Rework", "Urban Angles", "Van Bree Media"])
    ...
    # টপ আর্টিস্ট লজিক (Default selection)
    if not df.empty:
    ...
    with col_left:
        ...
        team_sum = df.groupby(['Team', 'Shift']).agg(
            ...
            Rework=('Product', lambda x: (x == 'Rework').sum()),
